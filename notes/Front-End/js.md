...
引用类型大小不固定，它会在堆内存中开辟空间存放数据，并在栈内存中存放其引用地址。

引用数据类型：先在栈内存中找到引用地址，再根据引用地址到堆内存中寻找数据，按引用访问。

## 栈

栈内存：有序排列，大小固定

基本数据类型大小固定不可修改，储存在栈内存中。

基本数据类型：直接返回栈内存中的数据，按值访问。
...
此外，在浏览器环境中，Error 对象具备 stack 属性, 该属性包含了错误的堆栈轨迹. 一个错误实例的堆栈轨迹包含了自构造函数之后的所有堆栈结构.
